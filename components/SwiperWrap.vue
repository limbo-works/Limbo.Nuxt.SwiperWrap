<!--
	Last modified: 2026/04/08 09:28:20
-->
<template>
  <Component :is="tag" class="c-swiper-wrap">
    <div
    ...
    <slot v-bind="slotBindings" name="custom">
      <slot v-bind="slotBindings" name="before" />
...
export default {
	...
	data() {
		return {
			...
			frameRequest: null,
			screenReaderText: '',
      _prevScrollLeft: 0, // eslint-disable-line
		};
	},
	computed: {
		slotBindings() {
	...
	methods: {
		initElements() {
			if (this.$slots.items) {
				...
				this.items = [...this.$el.querySelectorAll('.js-swiper-wrap__child')];
				this.spacerItems = [
					...this.$el.querySelectorAll('.js-swiper-wrap__child--spacer'),
				];
			...
			if (this.items?.length) {
				...
				console.warn('SwiperWrap: No content provided through \'items\' slot');
			}
		},
		...
		goto(index, behavior = 'smooth') {
			if (this.itemsContainer && this.items?.length) {
				// Set index
				const nextIndex = Math.min(Math.max(0, index), this.items.length - 1);
		...
		nextItems(loopAround) {
			if (this.itemsContainer) {
				...
				const nextIndex = loopAround
					? 0
					: Math.min(this.currentIndex + moveBy, this.items.length - 1);
		...
		resizeUpdate() {
			if (this.itemsContainer) {
				const { left, right } = this.itemsContainer.getBoundingClientRect();
		...
		goToFocused(e) {
			...
			if (this.itemsContainer && contentIndex >= 0) {
				const node = path[contentIndex - 1];
				const nodeList = [...this.itemsContainer.childNodes].filter(
					(node) => node && ![3, 8].includes(node.nodeType),
				);
				const nodeIndex = nodeList.indexOf(node);

				if (this.currentIndex !== nodeIndex) {
					this.itemsContainer.scrollLeft = this._prevScrollLeft;
					this.goto(nodeIndex);
				}
			}
		},
		...
		_childPartiallyVisible(item) {
			...
			return (
				isEncompassing || checkXWithin(rect.left) || checkXWithin(rect.right)
			);
		},
		_prepareFocusChange() {
			if (this.itemsContainer) {
				this._prevScrollLeft = this.itemsContainer.scrollLeft;
			}
		},
	},
};
